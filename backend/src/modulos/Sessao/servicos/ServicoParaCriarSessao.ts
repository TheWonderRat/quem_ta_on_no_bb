...
import {
  GerenciadorDeSenha,
  GerenciadorDeAutenticacao,
} from '../../../compartilhados/utilitarios/exporter';

export default class ServicoParaCriarSessao extends AbstractService
  < RequisicaoParaCriarSessao, RespostaParaCriarSessao > {
  override async executar(
    parameters: RequisicaoParaCriarSessao,
  ): Promise< RespostaParaCriarSessao | AppError > {
    //  checa se o usuario existe na base de dados
    const aprovado = await AprovadoRepo.buscarPorPosicaoAmpla(parameters.login);
    //  checa se o usuario existe na base de dados
    ...
    const token = GerenciadorDeAutenticacao.gerarToken(parameters.login.toString());
